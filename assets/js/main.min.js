...
(function(){const modal=document.getElementById("detail-modal");if(!modal)return;const $=(sel,root=modal)=>root.querySelector(sel);const $$=(sel,root=modal)=>Array.from(root.querySelectorAll(sel));const els={title:$("#detail-title"),summary:$("#detail-summary"),meta:$("#detail-meta"),img:$("#detail-img"),type:$("#detail-type"),aliases:$("#detail-aliases"),episodes:$("#detail-episodes"),closeBtns:$$("[data-close]"),};let lastFocus=null;function findItem(type,id){if(type==="villain"){return HOUSES.find((item)=>item.id===id);}
return WIZARDS.find((item)=>item.id===id);}
function listToLis(arr){if(!Array.isArray(arr)||!arr.length){return"<li>—</li>";}
return arr.map((value)=>`<li>${value}</li>`).join("");}
function renderMeta(item,type){if(type==="villain"){const parts=[item.founder?`<span class="badge">Founder: ${item.founder}</span>`:"",Array.isArray(item.colors)?item.colors.map((color)=>`<span class="badge">${color}</span>`).join(" "):"",item.ghost?`<span class="badge">Ghost: ${item.ghost}</span>`:"",];return parts.filter(Boolean).join(" ");}
const parts=[item.house?`<span class="badge">House: ${item.house}</span>`:"",Array.isArray(item.years)?item.years.map((year)=>`<span class="badge">${year}</span>`).join(" "):"",item.spoilerLevel==="high"?`<span class="badge">Spoilers</span>`:"",];return parts.filter(Boolean).join(" ");}
function getSummary(item,type){if(type==="character"&&item.spoilerLevel==="high"&&!state.showSpoilers){return"Spoilers hidden. Enable spoilers to reveal this summary.";}
return item.summary||"";}
function renderAliases(item,type){if(type==="villain"){return listToLis(item.traits);}
return listToLis(item.aliases);}
function renderEpisodes(item,type){if(type==="villain"){const details=[item.mascot?`Mascot: ${item.mascot}`:null,item.relic?`Relic: ${item.relic}`:null,item.ghost?`House Ghost: ${item.ghost}`:null,].filter(Boolean);return listToLis(details);}
return listToLis(item.notableEvents);}
function openModal(type,id,trigger){const item=findItem(type,id);if(!item)return;lastFocus=trigger||document.activeElement;els.title.textContent=item.name||"Details";els.summary.textContent=getSummary(item,type);els.img.src=item.img||"";els.img.alt=item.name?`${item.name} image`:"Detail image";els.type.textContent=type==="villain"?"House":"Wizard";els.meta.innerHTML=renderMeta(item,type);els.aliases.innerHTML=renderAliases(item,type);els.episodes.innerHTML=renderEpisodes(item,type);if(type==="villain"){els.type.style.borderColor="rgba(255,255,255,.12)";els.type.style.color="#CFCFCF";}else{els.type.style.borderColor="#8ab4f8";els.type.style.color="#E6EDF3";}
modal.classList.remove("is-hidden");document.body.style.overflow="hidden";const closeBtn=modal.querySelector(".modal__close");closeBtn?.focus();}
function closeModal(){modal.classList.add("is-hidden");document.body.style.overflow="";if(lastFocus&&typeof lastFocus.focus==="function"){lastFocus.focus();}}
document.addEventListener("click",(event)=>{const button=event.target.closest(".view-btn");if(!button)return;const id=button.getAttribute("data-id");const type=(button.getAttribute("data-type")||"").toLowerCase();if(!id||(type!=="villain"&&type!=="character"))return;event.preventDefault();openModal(type,id,button);});els.closeBtns.forEach((btn)=>{btn.addEventListener("click",closeModal);});modal.addEventListener("click",(event)=>{if(event.target.dataset.close==="backdrop"){closeModal();}});document.addEventListener("keydown",(event)=>{if(modal.classList.contains("is-hidden"))return;if(event.key==="Escape"){closeModal();}
if(event.key==="Tab"){const focusables=Array.from(modal.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')).filter((el)=>!el.hasAttribute("disabled"));if(!focusables.length)return;const first=focusables[0];const last=focusables[focusables.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus();}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus();}}});})();